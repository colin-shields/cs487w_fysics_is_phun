/**
 * DeckUploadCard.jsx
 * Host Deck Upload UI (final component, MVP behavior).
 *
 * Option 2 behavior (confirmed):
...
export default function DeckUploadCard() {
  ...
  // Stores the last upload response in a consistent structure
  const [result, setResult] = useState(null);

  // Stores the parsed deck rows from the last successful upload (if any)
  ...
  async function onUpload() {
    ...
    const res = await uploadDeckCsv(file);
    setResult(res);

    // Backend success shape (confirmed by you):
    // { status: "success", data: [ {Question_ID,...}, ... ] }
    if (res.ok && res.data && typeof res.data === "object") {
      if (res.data.status === "success" && Array.isArray(res.data.data)) {
        setUploadedDeckPreview(res.data.data);
  ...
  const backendStatus =
    backendData && typeof backendData === "object" ? backendData.status : null;
  const backendMessage =
    backendData && typeof backendData === "object" ? backendData.message : null;
    ...
    <section className="rounded-xl border border-slate-800 bg-slate-900/40 p-5">
      ...
      {result && (
        <div className="mt-4">
          ...
          {/* Raw response (kept for debugging during integration) */}
